...
st.write(f"Erreur quadratique moyenne (MSE) Decision Tree Regressor : {dt_mse}")
st.write(f"Erreur quadratique moyenne (MSE) Random Forest Regressor : {rf_mse}")
st.write(f"Erreur quadratique moyenne (MSE) XGBRegressor : {xgb_mse}")

# Tracer les résultats pour Decision Tree Regressor
...
# Tracer les résultats pour Random Forest Regressor
plt.figure(figsize=(20, 10))
plt.scatter(train_data['year'], train_data['anomalie'], color='blue', label='Données d\'entraînement')
plt.scatter(test_data['year'], test_data['anomalie'], color='red', label='Données de test')
plt.plot(test_data['year'], rf_predictions, color='green', label='Prédictions Random Forest Regressor')
plt.title('Random Forest Regressor pour les anomalies de température (1980-2021)')
plt.xlabel('Année')
plt.ylabel('Anomalie de température')
plt.legend()
plt.grid(True)
...
# Tracer les résultats pour XGBRegressor
plt.figure(figsize=(20, 10))
plt.scatter(train_data['year'], train_data['anomalie'], color='blue', label='Données d\'entraînement')
plt.scatter(test_data['year'], test_data['anomalie'], color='red', label='Données de test')
plt.plot(test_data['year'], xgb_predictions, color='orange', label='Prédictions XGBRegressor')
plt.title('XGBRegressor pour les anomalies de température (1980-2021)')
plt.xlabel('Année')
plt.ylabel('Anomalie de température')
plt.legend()
plt.grid(True)
...
# Évaluez les performances des modèles sur les données de test
dt_mse = mean_squared_error(y_test, dt_predictions)
rf_mse = mean_squared_error(y_test, rf_predictions)
xgb_mse = mean_squared_error(y_test, xgb_predictions)

print(f"Erreur quadratique moyenne (MSE) Decision Tree Regressor : {dt_mse}")
print(f"Erreur quadratique moyenne (MSE) Random Forest Regressor : {rf_mse}")
print(f"Erreur quadratique moyenne (MSE) XGBRegressor : {xgb_mse}")
...
# Divisez les données en ensembles d'entraînement et de test
train_data = df_anoIncTemp[(df_anoIncTemp['year'] >= 1950) & (df_anoIncTemp['year'] <= 2010)]
test_data = df_anoIncTemp[(df_anoIncTemp['year'] >= 2011) & (df_anoIncTemp['year'] <= 2021)]
